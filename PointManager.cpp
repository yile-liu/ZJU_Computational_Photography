...
void PointManager::init(const vector<vector<Point>> &structure_lines, const Mat1b &mask, int block_size,
                        set<shared_ptr<list<int>>> &line_set) {
    // clean old data
    ...
    for (map_iter = intersect_map.begin(); map_iter != intersect_map.end(); map_iter++) {
        ...
        for (list_iter = map_iter->second.begin(); list_iter != map_iter->second.end(); list_iter++) {
            if (line_set_record[list_iter->line_index] != nullptr) {
                ...
                {
                    ptr->insert(ptr->end(), line_set_record[list_iter->line_index]->begin(),
...
void PointManager::getPointsInPatch(const PointPos &p, list<Point *> &begin, list<int> &length) {
    ...
    for (auto &pointPosition: pointPositions) {
        int true_line_index = (in_mask) ? line_ends[pointPosition.line_index].true_line_index
                                        : pointPosition.line_index;
        ...
        for (int i = pointPosition.point_index; i >= 0; i--) {
            if (points[i].x < leftBound || points[i].y < upBound || points[i].x >= rightBound ||
                points[i].y >= downBound) {
                beginIndex = i + 1;
                break;
            }
        }
...
int PointManager::addNeighbor(MyNode &n, const PointPos &pos, vector<vector<ushort>> &visited_points,
                              list<shared_ptr<MyNode>> &bfs_stack) {
...
void PointManager::getStackReverseIter(list<shared_ptr<MyNode>>::reverse_iterator &begin,
                                       list<shared_ptr<MyNode>>::reverse_iterator &end) {
